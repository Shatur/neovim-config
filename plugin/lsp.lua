...
local function setup_rust_keymaps(buffer)
  ...
  vim.keymap.set({ '', 'i' }, '<F7>', function() rust.cargo('make') end, { noremap = true, buffer = buffer })
  vim.keymap.set({ '', 'i' }, '<S-F7>', function() rust.set_args('make') end, { noremap = true, buffer = buffer })
